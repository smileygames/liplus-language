...
これは最少構成のRequirements Specificationであり、Li+言語の最小コードである。issueテンプレートは単なる記入欄ではない。AIが実装に着手する前に、何を作るのか、何を守るのか、どうなれば終わりかを固定するための構文である。

### 完全版のRequirements Specificationが本体コードである

ただし、本腰はこの最小構文ではない。Li+言語の本体コードは完全版のRequirements Specificationである。

issueテンプレートはコンパイルを始めるための骨格にすぎない。背景、期待する挙動、守るべき判断、受け入れ条件が十分に書かれた完全版のRequirements Specificationがあってはじめて、セッションをまたいでも同じ要求から同じプログラムに収束しやすくなる。実装方法に差があっても、満たすべき要求が固定されていれば、生成されるプログラムの意味は揃えられる。

その完全版はdocs配下にある特定のRequirements Specification文書として管理する。docs全体がそのままRequirements Specificationになるわけではない。

### なぜRequirements Specificationがコードになるのか

Requirements Specificationは、人間とAIの外部記憶である。特に記憶を引き継げないAIにとっては、次のセッションでも同じ判断を再現するためのコードそのものになる。
...
Li+言語のコンパイルエラーとは、Requirements Specificationを読んでも、AIが実装フェーズへ進めない、または完了条件まで到達できないと判断される状態である。大きく分けると、次の2系統になる。
