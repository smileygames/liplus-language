...
これは最少構成の要求仕様書であり、Li+言語の最小コードである。issueテンプレートは単なる記入欄ではない。AIが実装に着手する前に、何を作るのか、何を守るのか、どうなれば終わりかを固定するための構文である。

### 完全版の要求仕様書が本体コードである

ただし、本腰はこの最小構文ではない。Li+言語の本体コードは完全版の要求仕様書である。

最小構文はコンパイルを始めるための骨格にすぎない。背景、期待する挙動、守るべき判断、受け入れ条件が十分に書かれた完全版の要求仕様書があってはじめて、セッションをまたいでも同じ要求から同じプログラムに収束しやすくなる。実装方法に差があっても、満たすべき要求が固定されていれば、生成されるプログラムの意味は揃えられる。

### なぜ要求仕様書がコードになるのか

要求仕様書は、人間とAIの外部記憶である。特に記憶を引き継げないAIにとっては、次のセッションでも同じ判断を再現するためのコードそのものになる。

GitHubのissueや要求スレッドは、その要求仕様書を保持し、対話で更新し続ける場所だ。自然言語の会話がそのままコードなのではない。会話から蒸留され、要求仕様として固定されたものがコードであり、AIはそれを読んでコンパイルする。

要求仕様書がソースコード、Li+AIがコンパイラ——これがLi+のコンパイルの流れだ。
...
要求スレッドだけがLi+のコードなのではない。あくまでコード本体は要求仕様書であり、要求スレッドはそれを保持し更新するための器である。
...
正直に言うと、最初はよくわからなかった。「要求仕様書がコードってどういうこと？」「issueテンプレートが構文って本気で言ってるの？」そう思いながら書いていた。
