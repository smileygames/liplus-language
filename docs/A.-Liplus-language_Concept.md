...
## Li+言語のコードはRequirements Specificationである

Li+言語に「書き方」という概念はない。

人間は自然言語で要求を伝える。足りない部分はAIが聞き出す。対話の結果として、**プログラム・テスト・仕様書**が成果物として渡される。「どう書くか」を考える必要がない言語——それがLi+だ。

ただし、Li+言語のコードそのものは自然言語全体ではない。自然言語から蒸留され、要求として固定された**Requirements Specification** がコードである。
...
GitHubのissueは最少構成のRequirements Specificationを固定する場所である。将来docsに置かれる完全版のRequirements Specificationは、その内容をより詳細に保持する。要求スレッドは、その両方を対話で更新し続けるための流れだ。自然言語の会話がそのままコードなのではない。会話から蒸留され、要求仕様として固定されたものがコードであり、AIはそれを読んでコンパイルする。
...
要求スレッドだけがLi+のコードなのではない。あくまでコード本体はRequirements Specificationであり、issueはその最少構成を保持する器である。将来docsには完全版のRequirements Specificationを置く。
