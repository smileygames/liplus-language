...
Claude セッションの内部ブランチ名をそのまま GitHub ブランチ名として使用する。
`{issue-number}-{slug}` の前に `claude/` プレフィックスを付ける。

```
claude/{issue-number}-{issue-title-slugified}
# 例: claude/365-github_wiki_updete
```

作成コマンド：

```
gh issue develop {issue_number} -R {owner}/{repo} --name claude/{issue-number}-{slug} --base main
```

セッション内部名 = GitHub ブランチ名。マッピング不要。

### 複数ファイルコミット時のツリー検証（必須）

tree 作成後、コミット前に必ずファイル数を検証する。

```
# NG: ファイル数が base_tree より減っていたらコミット中止
FILE_COUNT=$(gh api repos/{owner}/{repo}/git/trees/{new-tree-sha}?recursive=1 \
  -q '[.tree[] | select(.type=="blob")] | length')
# FILE_COUNT >= BASE_FILE_COUNT であることを確認してからコミット
```

ファイル数が減少している場合は `base_tree` の指定ミスの可能性があるため、必ず中止して確認する。