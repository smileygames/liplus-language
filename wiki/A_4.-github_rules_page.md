...
-   親 issue の body はタスク全体の目的を簡潔に記述
-   詳細な完了条件は各子 issue に記述
-   commit body で子 issue 番号を `Refs #xxx` 参照する
...
-   `closes` などのキーワードは不要。`Refs #xxx` の形式で自動クローズされる
...
2.  **結論判定**
    ...
    -   全て `success` の場合 → CI PASS → レビュー依頼
3.  **CI FAIL 時の自動修正ループ（ループ安全制約付き）**
    -   修正を試みて新規コミットで CI を再実行
    -   同一の修正アプローチを3回繰り返したら中止する
    -   それでも収束しない場合は自動修正を停止し、Issue にコメントとして外部化して人間に委ねる
